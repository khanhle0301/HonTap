...
namespace HonTap.Model.Dao
{
    public class PostDao
    {
        ...
        {
            var query = db.Posts.Include("PostCategory").Where(x => x.Status && x.Name.Contains(keyword));
        ...
        public IEnumerable<Post> GetAllByCategoryPaging(int categoryId, int page, int pageSize, out int totalRow)
        {
            ...
            var category = db.PostCategories.Find(categoryId);
            var childCategories = db.PostCategories.Where(x => x.ParentID == category.ID);
            foreach (var item in childCategories)
            {
                query = query.Concat(db.Posts.Include("PostCategory").Where(x => x.Status && x.CategoryID == item.ID));
            }
            query = query.Concat(db.Posts.Include("PostCategory").Where(x => x.Status && x.CategoryID == categoryId)).Distinct();
            totalRow = query.Count();
            return query.OrderByDescending(x => x.UpdatedDate).Skip((page - 1) * pageSize).Take(pageSize);
        }
        ...
        {
            var query = db.Posts.Include("PostCategory").Where(x => x.Status).OrderByDescending(x => x.UpdatedDate);
            totalRow = query.Count();
        ...
        public IEnumerable<Post> GetViewCount(int top)
        {
            return db.Posts.Include("PostCategory").Where(x => x.Status).OrderByDescending(x => x.ViewCount).Take(top);
        }
